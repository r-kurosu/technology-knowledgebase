...
## Skills vs Tools（使い分け）

名前が紛らわしいが**レイヤーが違う**。**Tool = Claude が「呼ぶ」もの**（動作そのもの／verb）、**Skill = Claude が「読む」もの**（手順書＋付属ファイル。動作は既存 Tool に乗る）。Skill は単体では何も実行できない。

> **肝**: Skill がスクリプトを同梱していても、それを動かすのは Skill 専用の実行経路ではなく **Bash ツール**。SKILL.md に「`python scripts/foo.py` を実行して」と書いてあり、Claude が Bash を呼んで走らせるだけ。だから Skill を有効化しても `Bash`（や `Read`）を `allowed_tools` に入れ忘れると、その付属スクリプトは動かない。

「推論力が上がってどちらでも同じことができそう」は **happy path の能力レベルでは真、保証レベルでは別物**。曖昧にならない軸が 4 つある。

| 軸 | Tool | Skill |
|---|---|---|
| **保証** | 固定契約（スキーマ検証済み入力 → 同じコード経路）。確定的 | Claude が指示を読み Bash コマンドを組む。非確定的（推論力では埋まらない） |
| **ブラスト半径** | その 1 機能だけを型付きで露出。Bash 不要 | スクリプト実行に `Bash`＝**任意シェル実行権**が要る |
| **住む場所** | アプリのコードに焼き込み | `.claude/` の宣言的アーティファクト（コード外） |
| **書ける人** | プログラマ必須 | Markdown なので非エンジニアも可 |

**Skill が勝つ**: アプリロジック/ビジネスロジックの分離（`.claude/` に集約）／ポータビリティ（CLI・SDK・claude.ai で同じものが動き git 共有可。Tool は定義したアプリに縛られる）／progressive disclosure（普段は name+description だけ＝多数持っても文脈ほぼ無料。Tool はスキーマが毎ターン載る）／柔軟性（手順を適応・付属スクリプトを書き換えて実行できる）。

**Tool が勝つ**: 確定性・冪等性（課金・DB書込・外部API）／最小権限（Bash を渡さず狭い能力 1 個だけ許可）／秘密隠蔽（ハンドラは自プロセス内で動くので API キー等を Claude にもサンドボックスにも見せない）／構造化 I/O（enum・必須・`structuredContent`・画像返却）／ホストアプリ統合（`isError` でループ継続・自言語ランタイム実行）。

> **柔軟性は諸刃**: Skill が手順を適応できる性質は、開発・探索では強みだが、**不可逆・要保証の操作では負債**。「Claude が Bash コマンドを組み立てて決済を叩く」は柔軟性が仇になる典型。

**落としどころは合成**: **Skill = オーケストレーション/知識層、Tool = 狭くて確実に効かせたい実行層**。

> 例）返金 Skill が「うちの返金ポリシーはこう、この手順で確認」と*判断込みの手順*を持ち、確実に効かせたい 1 操作だけを狭いカスタム Tool `issue_refund(order_id, amount)`（サーバー側認証で決済 API を叩く）に委ねる。Skill の分離・柔軟性・ポータビリティと、Tool の確定性・最小権限・秘密隠蔽を両取りできる。「不可逆・要保証の狭い操作を `Skill＋Bash` に任せない」が実務の勘所。

> **目安**: 個人・軽量用途や手順/グルーは **Skill 中心で十分**。確定性・秘密・最小権限が要る狭い操作だけ **Tool** を足す。
