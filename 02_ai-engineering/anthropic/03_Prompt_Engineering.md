...
> **Claude 4.6 以降（Opus 4.6 / 4.7 / 4.8・Sonnet 4.6・Fable 5）では、最後のアシスタントターンでのプリフィルはサポートされておらず 400 エラーになる。** 代替として構造化出力（`output_config.format`）やシステムプロンプト指示を使用すること。

---

## 12. 拡張思考 (Extended Thinking)

- 旧方式では `budget_tokens` で思考予算を制御（最小1,024トークン）
- **`budget_tokens` の扱い（重要）**:
  - Claude Opus 4.6 / Sonnet 4.6: **非推奨**（まだ動作する移行用エスケープハッチ）
  - Claude Fable 5 / Opus 4.8 / 4.7: **削除済み**。指定すると 400 エラー
  - 代わりに `thinking: {type: "adaptive"}`（Adaptive Thinking）＋ `effort` パラメータで深さを制御
...
**ポイント**: `any` または `tool` を指定すると、Claudeはツール使用前に自然言語の応答を生成**しない**。
...
1. **明示的であれ（Be Explicit）** — 曖昧さを避け、基準・エッジケースを最も明示的に定義する
2. **confidence-based の指示は機能しない** — Claudeに校正された信頼度シグナルはないため、「自信があるときだけ」は失敗する
3. **リトライは具体的なエラー情報を含める** — 汎用リトライメッセージは避け、フィールド・エラー内容・期待値を添える
4. **プロンプトベース vs スキーマ強制** — 正規表現でのJSONパースは脆弱。スキーマ強制（構造化出力）を優先する
5. **nullableフィールドがハルシネーション削減に役立つ** — データがない場合に `null` を許容する
