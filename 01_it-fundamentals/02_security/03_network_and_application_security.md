...
**落とし穴の例**：CloudFrontにWAFを設定していても、ALBのIPが公開されていれば直接叩いてCloudFrontを迂回できる。
```
通常：Internet → CloudFront(WAF) → ALB → App  ← WAFが効く
迂回：Internet ──────────────────→ ALB → App  ← WAFをスキップ
```
対策：ALBのSecurity GroupをCloudFrontのIPからの通信のみ許可し、インターネットから直接到達できないようにする。「エッジを守っても、オリジンへの直接経路を閉じないと迂回される」。
...
初手からBlockモードにすると、正常サービスを止めるリスクがある。例えばECサイトの商品検索クエリがSQLiルールに誤検知され、検索機能が丸ごとブロックされる、といった事故が起こりうる。
