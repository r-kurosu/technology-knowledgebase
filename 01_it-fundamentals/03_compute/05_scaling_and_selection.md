...
| 論点 | EC2 | ECS | EKS | Lambda |
|---|---|---|---|---|
| 制御の自由度 | 高（OS/ミドルウェアまで） | 中（コンテナ単位） | 中〜高（Kubernetesエコシステムを制御できる） | 低（コードのみ） |
| 運用責任 | すべて自分（パッチ、スケーリング設計） | 一部AWS（Fargateならホスト管理不要） | コントロールプレーンはAWS、ノード/Kubernetes運用は自分 | ほぼAWS |
| 応答速度（起動の速さ） | 起動済みインスタンスは即応、新規起動は遅い | 起動済みタスクは即応 | 起動済みPodは即応 | コールドスタートの可能性あり |
| 実行時間上限 | なし | なし | なし | 15分 |
| アイドル容量（コスト） | 常時起動分のコストが発生 | Fargateなら実行中のみ課金、常駐設計だとアイドルコストも発生 | ノード分のアイドルコストが発生しうる | アイドル時は課金なし |
| 予測可能性 | 高（容量を自分で確保できる） | 中 | 中 | 低〜中（自動スケールだが下流DB接続数枯渇等に注意、[04](04_serverless_and_batch.md)参照） |

基本方針：制御と運用負荷はトレードオフの関係にある。細かい制御（OSレベルの調整、特殊なミドルウェア）が必要ならEC2、コンテナ化された常駐サービスで運用負荷を下げたいならECS、Kubernetesエコシステム（マルチクラウド前提、既存の資産）が必要ならEKS、トラフィックが不定期・短時間で運用負荷を最小化したいならLambda、という整理になる。
