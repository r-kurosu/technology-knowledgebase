...
小〜中規模で信頼できる範囲が閉じているチームでは、Pull型（GitOps）の常時Drift監視の恩恵より、追加のエージェント運用が不要なPush型のシンプルさが実務上勝りやすい。規模が大きくクラスタが分散するほど、Pull型の自己修復の価値が相対的に上がる。

### 品質ゲートの置き場所（並列化とコスト）

品質ゲート（実環境での検証）を「どこで・何個」置くかはコストとのトレードオフになる。

- マージ後に共有のdev環境で検証：環境は1つで済みコストは低いが、誰かの変更がdev環境を壊すと他の作業者の検証もブロックされる（共有資源の奪い合い）
- 作業ブランチごとに検証環境を並列で用意（プレビュー環境）：1つが壊れても他に影響しないが、ブランチの数だけ環境を用意・破棄するコストと複雑さが増える

速度・品質ゲートの厳密さを取るほど、コストと運用の複雑さが増えるという軸で並列化を捉えられる。

### 再現性（Build once, deploy many vs 再ビルド）

devとmainで別々にBuildを走らせる再ビルド型は、パイプラインの構成がシンプルな反面、ビルドの再現性が完全でないと「devで検証したもの」と「本番で動くもの」が別物になりうる。Artifactを1回だけビルドして使い回す構成にすれば再現性は上がるが、環境差分（設定値等）をイメージの外側で管理する設計が追加で必要になる。

### 承認（Approval）の価値

承認ゲートの価値は、寄せられる人の判断が実質的に機能しているかに依存する。自動テスト・dev環境検証が既に強い品質ゲートとして機能している場合、承認は「自動テストでは拾えない人の判断（リリースタイミング等）」を確認する場合にのみ追加の価値を持つ。それが形骸化している（内容を見ずApprove）なら、あってもなくても実質的な差はない。
